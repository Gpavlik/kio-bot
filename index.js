...
async function syncUsersFromSheet() {
  try {
    const response = await axios.get('https://script.google.com/macros/s/AKfycby1A3D-ckgWzoJpxuZaRA9_A_TAGyT22ZmYoLqTioOmJUNvOmzexRd79-X8_U9DQzXf/exec?action=getUsers');
...
bot.onText(/📜 Історія замовлень/, async (msg) => {
  ...
  try {
    const res = await axios.post('https://script.google.com/macros/s/AKfycby1A3D-ckgWzoJpxuZaRA9_A_TAGyT22ZmYoLqTioOmJUNvOmzexRd79-X8_U9DQzXf/exec', {
      action: 'getHistory',
      chatId
    });
...
bot.onText(/📊 Статистика/, async (msg) => {
  ...
  try {
    const [orderRes, userRes] = await Promise.all([
      axios.post('https://script.google.com/macros/s/AKfycby1A3D-ckgWzoJpxuZaRA9_A_TAGyT22ZmYoLqTioOmJUNvOmzexRd79-X8_U9DQzXf/exec', { action: 'getStats' }),
      axios.post('https://script.google.com/macros/s/AKfycby1A3D-ckgWzoJpxuZaRA9_A_TAGyT22ZmYoLqTioOmJUNvOmzexRd79-X8_U9DQzXf/exec', { action: 'getUserOrderStats' })
    ]);
...
bot.on('callback_query', async (query) => {
  ...
  const SCRIPT_URL = 'https://script.google.com/macros/s/AKfycby1A3D-ckgWzoJpxuZaRA9_A_TAGyT22ZmYoLqTioOmJUNvOmzexRd79-X8_U9DQzXf/exec';
  ...
  if (data.startsWith('verify_')) {
    ...
    try {
      await axios.post('https://script.google.com/macros/s/AKfycby1A3D-ckgWzoJpxuZaRA9_A_TAGyT22ZmYoLqTioOmJUNvOmzexRd79-X8_U9DQzXf/exec', {
        action: 'addUser',
        name: request.name,
  ...
  // ✉️ Повідомлення користувачу (оператор відповідає конкретному користувачу)
  if (data.startsWith('msg_')) {
    const targetId = data.split('_')[1];
    pendingMessage[adminId] = targetId;
    await bot.sendMessage(adminId, `✍️ Введіть повідомлення для користувача ${targetId}:`);
 ...
 if (data.startsWith('accept_')) {
  ...
  if (data.startsWith('cancel_')) {
    ...
    try {
      ...
      await bot.sendMessage(adminId, `❌ Замовлення від @${user?.username || 'невідомо'} було скасовано.`);
...
if (data === 'payment_cod' || data === 'payment_prepaid') {
  ...
  try {
    await axios.post('https://script.google.com/macros/s/AKfycby1A3D-ckgWzoJpxuZaRA9_A_TAGyT22ZmYoLqTioOmJUNvOmzexRd79-X8_U9DQzXf/exec', {
      ...
      np: order.np,
      phone: order.phone,
      paymentMethod: order.paymentMethod, // 👈 нове поле
      status: order.status,
      date: order.date,
      time: order.time
    });
...
if (data.startsWith('paid_')) {
  ...
  try {
    ...
    await bot.sendMessage(chatId, `💳 Ваше замовлення позначено як *оплачене*. Дякуємо!`, { parse_mode: 'Markdown' });
    await bot.sendMessage(adminId, `✅ Статус оновлено: *оплачено* для ${order.name || 'користувача'} (${chatId})`, { parse_mode: 'Markdown' });
  } catch (err) {
    console.error('❌ Помилка оновлення статусу:', err.message);
    await bot.sendMessage(adminId, `❌ Помилка оновлення статусу: ${err.message}`);
  }
