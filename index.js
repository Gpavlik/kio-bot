...
// 🛠 Дія
if (text === '🛠 Дія') {
  bot.sendMessage(chatId, `Остеоартрит — дегенеративне захворювання, що супроводжується болем і тертям у суглобах. KioMedinevsOne — внутрішньосуглобовий засіб на основі хітозану, який знижує тертя, нейтралізує вільні радикали та зменшує окисний стрес.`);
  return;
}

// 📦 Склад
if (text === '📦 Склад') {
  bot.sendMessage(chatId, `Упаковка містить:\n• 1 шприц з 3 мл стерильного розчину\n• 20 мг хітозану, 35 мг сорбіту\n• Фосфатно-буферна вода (рН 7.2 ± 0.2)\n• Інструкція та етикетки`);
  return;
}

// ⚙️ Ефект
if (text === '⚙️ Ефект') {
  bot.sendMessage(chatId, `Одна ін'єкція забезпечує:\n• Зменшення болю та скутості\n• Покращення функціональності суглоба\n• Ефект до 6 місяців\n• Можливість повторного курсу за рекомендацією лікаря`);
  return;
}

// ⚠️ Увага
if (text === '⚠️ Увага') {
  bot.sendMessage(chatId, `• Не вводити при синовіальному випоті\n• Не застосовувати у вагітних, дітей, при інших захворюваннях\n• Зберігати при 2–25 °C, не заморожувати\n• Одноразове використання\n• Дотримання асептики обов’язкове\n• Високий тиск при ін’єкції — ознака неправильного положення голки`);
  return;
}

// 📝 Застосування
if (text === '📝 Застосування') {
  bot.sendMessage(chatId, `• Ін'єкція виконується лікарем з досвідом\n• Перед введенням — обробка антисептиком\n• Голка: Люер 20G–23G\n• Можливе УЗД-контролювання\n• Після ін'єкції — холод, спокій, НПЗП\n• Утилізація голки — у контейнер для гострих предметів`);
  return;
}
